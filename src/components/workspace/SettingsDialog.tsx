...
const allTags = ['全部', '产品设计', '编码', '需求', '质量', '测试', '数据库', '优化', '调试', '设计'];
...
function AgentModelCard({
  ...
  agent: AgentInstance;
  onUpdate: (agents: AgentInstance[]) => void;
  allAgents: AgentInstance[];
}) {
  const [showKey, setShowKey] = useState(false);
  const config = agent.modelConfig || { type: 'builtin', modelId: 'gpt-4' };

  const updateConfig = (patch: Partial<typeof config>) => {
    const newConfig = { ...config, ...patch };
    onUpdate(
      allAgents.map((a) =>
        a.id === agent.id ? { ...a, modelConfig: newConfig } : a
      )
    );
  };
      ...
      <div className="flex items-center gap-2">
        {[
          ...
          <button
            key={opt.key}
            type="button"
            onClick={() => updateConfig({ type: opt.key as 'builtin' | 'custom' })}
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded border border-border bg-secondary text-muted-foreground hover:text-foreground transition-colors shrink-0"
                title={showKey ? '隐藏' : '显示'}
              >
  ...
  return (
    <div className="space-y-3 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {specTab === 'global' && (
          <div className="space-y-2">
            <Label className="text-sm font-normal">全局约束</Label>
...
- 关键路径有降级方案`}
        ...
        {specTab === 'engineering' && (
          <div className="space-y-2">
            <Label className="text-sm font-normal">工程约束</Label>
            <div className="rounded border border-border bg-secondary/30 p-3">
              <pre className="text-xs font-mono text-muted-foreground whitespace-pre-wrap leading-relaxed">
{`# 工程约束
...
- src/types/ - 类型定义`}
        ...
        {specTab === 'visual' && (
          <div className="space-y-2">
            <Label className="text-sm font-normal">视觉约束</Label>
            <div className="rounded border border-border bg-secondary/30 p-3">
              <pre className="text-xs font-mono text-muted-foreground whitespace-pre-wrap leading-relaxed">
{`# 视觉约束
...
export default function SettingsDialog({ open, onOpenChange, agents = [], onUpdateAgents }: SettingsDialogProps) {
  ...
  const filteredPrompts = promptCards.filter((card) => {
  ...
  const handleCopyPrompt = (card: { id: string; title: string; content: string }) => {
  ...
  const menuItems = [
    ...
    { id: 'specs' as const, label: '工程规范', icon: BookOpen },
            ...
            {activeSetting === 'agents' && (
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto">
                  {agents?.map((a) => (
                    <AgentModelCard key={a.id} agent={a} onUpdate={onUpdateAgents} allAgents={agents} />
                  ))}
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {filteredPrompts.length === 0 ? (
                    ...
                    filteredPrompts.map((card) => (
                      <div
                        key={card.id}
                        className="p-3 rounded border border-border bg-secondary/30 hover:bg-secondary/50 transition-colors"
                      >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-[11px] rounded bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
                  >
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Model Context Protocol (MCP) 允许智能体通过标准化接口与外部工具和数据源交互。
                </p>
