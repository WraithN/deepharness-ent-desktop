...
impl DbService {
    ...
    pub fn sign_in(&self, username: String, _password: String) -> Result<AuthUser, String> {
    ...
    pub fn sign_up(&self, username: String, _password: String) -> Result<AuthUser, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
    ...
    pub fn load_messages(&self, conversation_id: String, limit: i64) -> Result<Vec<Value>, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let mut stmt = conn
